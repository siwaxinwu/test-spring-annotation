...
import com.roy.annotation.EnableSpringStudy;
 ...
 * description：
 * author：dingyawu
 * date：created in 23:17 2020/12/4
 * history:
 */
public class TestAnnotation {
    ...
    public void testAnnotationConfig4(){
        AnnotationConfigApplicationContext ac = new AnnotationConfigApplicationContext(PersonConfig5.class);
        //向容器中注册自定义的scope
    ...
    public void testEnableSpringStudy(){
        AnnotationConfigApplicationContext applicationContext = new AnnotationConfigApplicationContext(OtherJarConfiguration.class);
    ...
    public void testMyImportBeanDefinitionRegistrar(){
        AnnotationConfigApplicationContext ac = new AnnotationConfigApplicationContext(PersonConfig10.class);
        String[] names = ac.getBeanDefinitionNames();
        Arrays.stream(names).forEach(System.out::println);
    }

    /**
     * 测试PersonFactoryBean这个实例
     * personFactoryBean实例的类型为：class com.roy.beans.Person,
